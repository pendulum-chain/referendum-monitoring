...
const OpenReferenda = () => {
  ...
  const fetchData = async (api) => {
    ...
    const currentBlockNumber = Number(lastHeader.toHuman().number.replaceAll(',',''))
  
    let extractedReferenda = data.map(([key, info]) => {
    ...
    <ul>
      {data.map((referenda, index) => (
        <li key={index}>
          ...
          <span>
              <a href={`${baseUrl}/referendum/${referenda.index}`} target="_blank" rel="noopener noreferrer">
